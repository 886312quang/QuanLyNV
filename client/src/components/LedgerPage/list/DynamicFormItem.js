...
import { Input, Row, Col, InputNumber, Button, Tooltip, Select } from "antd";
import selectors from "../../../_selectors/ledger";
import staffSelectors from "../../../_selectors/staff";
      ...
      <Row key={key} style={{ margin: "5px 0px" }}>
        ...
        <Col xs={24} md={4} style={{ padding: "0px 4px" }}>
          <InputNumber
            ...
            name="certificate"
            onChange={(value) => handleChange("certificate", value, key)}
        ...
        <Col xs={24} md={6} style={{ padding: "0px 4px" }}>
          <Tooltip title="Đánh dấu?">
            <Button
              onClick={(_) => handleChange("flag", !service.flag, key)}
          ...
          <Tooltip title="Xóa">
            <Button
              onClick={() => dispatch(actions.doServiceItemRemoveClick(key))}
              icon="minus"
            />
          </Tooltip>
          <Tooltip title="Thêm">
            <Button
              onClick={() => dispatch(actions.doServiceItemAddClick(key))}
              icon="plus"
            />
          </Tooltip>
        </Col>
